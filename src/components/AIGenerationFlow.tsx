import { ArrowLeft, Bath, Bed, Briefcase, ChefHat, DoorOpen, Home, Maximize2, MessageSquare, Minimize2, Minus, Plus, RefreshCw, Upload, UtensilsCrossed, X } from 'lucide-react';
...
export function AIGenerationFlow({ initialSpaceName = '', onBack, onFullViewChange }: AIGenerationFlowProps) {
  ...
  const [chatMessages, setChatMessages] = useState<{ id: string; text: string; sender: 'user' | 'ai'; timestamp: string }[]>([]);
  ...
  const [layoutSketch, setLayoutSketch] = useState<string | null>(null);
  ...
  const shoppingItems = [
    { id: '1', name: 'Scandinavian Oak Sofa', price: 1299, retailer: 'West Elm', size: '220 x 90 cm', finish: 'Natural Oak' },
    { id: '2', name: 'Linen Accent Chair', price: 549, retailer: 'Article', size: '80 x 85 cm', finish: 'Off-white Linen' },
    { id: '3', name: 'Walnut Coffee Table', price: 429, retailer: 'CB2', size: '120 x 60 cm', finish: 'Walnut Veneer' },
    { id: '4', name: 'Woven Area Rug', price: 349, retailer: 'Rugs USA', size: '240 x 170 cm', finish: 'Natural Jute' },
    { id: '5', name: 'Arc Floor Lamp', price: 189, retailer: 'West Elm', size: '180 cm height', finish: 'Brass' },
    { id: '6', name: 'Ceramic Vase Set', price: 79, retailer: 'CB2', size: 'Assorted', finish: 'Matte Cream' },
  ];

  const refineFilters = [
    'Seating', 'Tables', 'Lighting', 'Textiles', 'Decor', 'Color palette', 'Materials'
  ];
  ...
  const handleLayoutUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setLayoutSketch(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
        ...
        <div className="absolute right-6 top-0 flex flex-col gap-6">
          {steps.map((step) => (
            ...
            >
              ...
              {/* Step Title - Always visible, light when inactive */}
              <span
        ...
        <div className="grid gap-8 pr-28 min-h-0" style={{ gridTemplateColumns: '1fr minmax(320px, 380px)' }}>
                ...
                <label htmlFor="image-upload">
                  ...
                  >
                    {uploadedImage ? (
                      ...
                      <>
                        <Upload size={32} className="text-[#c5c5c5] mb-6" strokeWidth={1.5} />
                        <p 
          ...
          {currentStep === 2 && (
            <div>
              ...
              <div>
                <div 
                  className="bg-[#FDFCFB] rounded-sm flex flex-col items-center justify-center text-center relative shadow-sm overflow-hidden"
                  style={{ height: '600px' }}
                >
                  {uploadedImage ? (
                    <>
            ...
            <div>
              <div>
                <label htmlFor="inspiration-upload">
                  <div 
                    className="bg-[#FDFCFB] rounded-sm flex flex-col items-center justify-center text-center cursor-pointer transition-colors duration-300 hover:bg-[#FAF9F7] shadow-sm"
                    style={{ height: '600px', padding: '64px' }}
                  >
                    <Upload size={32} className="text-[#c5c5c5] mb-6" strokeWidth={1.5} />
                    <p 
                      className="text-[18px] text-[#c5c5c5] mb-2"
                    ...
                    <p 
                      className="text-[12px] text-[#d5d5d5] mb-6"
                    ...
                    <input
                      ...
                      className="w-full max-w-xs px-0 py-2 text-[14px] text-[#2a2a2a] placeholder:text-[#c5c5c5] focus:outline-none bg-transparent border-b border-[#E8E6E3] focus:border-[#A4AC96] transition-colors duration-500"
                      style={{ fontFamily: "'Inter', sans-serif", fontWeight: 300 }}
                      onClick={(e) => e.stopPropagation()}
                    />
                  </div>
                </label>
                <input id="inspiration-upload" type="file" accept="image/*" multiple className="hidden" />
              </div>
            </div>
          )}
              ...
              <div>
                ...
                >
                  {uploadedImage ? (
                    ...
                    <p className="text-[14px] text-[#c5c5c5]" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 300 }}>
                      No image uploaded
                    </p>
                  )}
                </div>
            ...
            <div>
              <div>
                <div className="relative">
                  ...
                  <div className="bg-[#FDFCFB] rounded-sm flex flex-col items-center justify-center text-center relative shadow-sm overflow-hidden" style={{ height: '600px' }}>
                    {uploadedImage ? (
                      <img src={uploadedImage} alt={showBeforeAfter ? "Before" : "After"} className="rounded-sm" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
                    ) : (
                      <p className="text-[14px] text-[#c5c5c5]" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 300 }}>No image uploaded</p>
                    )}
                    {uploadedImage && !showBeforeAfter && (
                      <p className="absolute bottom-6 left-1/2 -translate-x-1/2 text-[11px] text-[#9a9a9a] bg-[#F7F5F2]/90 backdrop-blur-sm px-3 py-2 rounded-sm" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 300, letterSpacing: '0.01em' }}>
            ...
            <div>
              <div>
                <div className="relative">
                  ...
                  <div className="bg-[#FDFCFB] rounded-sm flex flex-col items-center justify-center text-center shadow-sm overflow-hidden" style={{ height: '600px' }}>
                    {uploadedImage ? (
                      <img src={uploadedImage} alt={showBeforeAfter ? "Before" : "Final design"} className="rounded-sm" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
                    ) : (
                      <p className="text-[14px] text-[#c5c5c5]" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 300 }}>No image uploaded</p>
                    )}
                  </div>
                    ...
                    >
                      {roomType ? (
                        <span className="flex items-center gap-3">
                          {(() => {
                            const Icon = roomTypeOptions.find(option => option.value === roomType)?.icon;
                            return Icon ? <Icon size={16} strokeWidth={1.5} className="text-[#9a9a9a]" /> : null;
                          })()}
                          <span className="text-[#2a2a2a]">{roomTypeOptions.find(option => option.value === roomType)?.label}</span>
                        </span>
                      ) : (
                        'Select room type'
                      )}
                      ...
                      >
                        {roomTypeOptions.map(option => {
                          ...
                          return (
                            ...
                            >
                              <Icon size={16} strokeWidth={1.5} className="text-[#9a9a9a]" />
              ...
              <div>
                ...
                {activeTab === 'controls' ? (
                  <div className="space-y-6">
                    <h3 
                      className="text-[18px] text-[#2a2a2a] mb-6"
                    >
                      Detected Elements
                    </h3>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button
                    ...
                    className={`transition-all duration-300 ${
                      selectedLayout === 1 ? 'opacity-100' : 'opacity-60 hover:opacity-80'
                    }`}
                  >
                    <div className="bg-[#FDFCFB] rounded-sm p-6 mb-2">
                      <div className="space-y-3">
                        <div className="h-12 bg-[#E8E6E3] rounded-sm" style={{ width: '60%' }}></div>
                        <div className="flex gap-3">
                          <div className="h-10 bg-[#E8E6E3] rounded-sm" style={{ width: '40%' }}></div>
                          <div className="h-10 bg-[#E8E6E3] rounded-sm" style={{ width: '40%' }}></div>
                        </div>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className={`transition-all duration-300 ${
                      selectedLayout === 2 ? 'opacity-100' : 'opacity-60 hover:opacity-80'
                    }`}
                  >
                    <div className="bg-[#FDFCFB] rounded-sm p-6 mb-2">
                      <div className="space-y-3">
                        <div className="flex gap-3">
                          <div className="h-10 bg-[#E8E6E3] rounded-sm" style={{ width: '50%' }}></div>
                          <div className="h-10 bg-[#E8E6E3] rounded-sm" style={{ width: '30%' }}></div>
                        </div>
                        <div className="h-8 bg-[#E8E6E3] rounded-sm" style={{ width: '70%' }}></div>
                      </div>
                    </div>
                    <p 
                      className="text-[12px] text-[#2a2a2a] text-left"
                  ...
                  <button
                    ...
                    className={`transition-all duration-300 ${
                      selectedLayout === 3 ? 'opacity-100' : 'opacity-60 hover:opacity-80'
                    }`}
                  >
                    <div className="bg-[#FDFCFB] rounded-sm p-6 mb-2">
                      <div className="space-y-3">
                        <div className="h-10 bg-[#E8E6E3] rounded-sm" style={{ width: '75%' }}></div>
                        <div className="h-10 bg-[#E8E6E3] rounded-sm" style={{ width: '45%' }}></div>
                      </div>
                    </div>
                    <p 
                      className="text-[12px] text-[#2a2a2a] text-left"
                  ...
                  <button
                    ...
                    className={`transition-all duration-300 ${
                      selectedLayout === 4 ? 'opacity-100' : 'opacity-60 hover:opacity-80'
                    }`}
                  >
                    <div className="bg-[#FDFCFB] rounded-sm p-6 mb-2">
                      <div className="space-y-3">
                        ...
                        <div className="h-6 bg-[#E8E6E3] rounded-sm" style={{ width: '80%' }}></div>
                      </div>
                    </div>
                    <p 
                      className="text-[12px] text-[#2a2a2a] text-left"
                ...
                <div className="pt-6">
                  ...
                  <label htmlFor="layout-upload">
                    <div 
                      className="bg-[#FDFCFB] rounded-sm p-6 flex flex-col items-center justify-center cursor-pointer transition-colors duration-300 hover:bg-[#FAF9F7] overflow-hidden"
                      style={{ minHeight: '100px', maxHeight: '200px', border: '1px dashed #E8E6E3' }}
                    >
                      {layoutSketch ? (
                        <div className="relative w-full h-full flex items-center justify-center">
                          ...
                          <button
                            ...
                            className="absolute top-1 right-1 p-1 bg-[#F7F5F2]/80 backdrop-blur-sm rounded-sm hover:opacity-60 transition-opacity duration-300"
                          >
                            <X size={12} className="text-[#626262]" strokeWidth={1.5} />
                          </button>
                        </div>
                      ) : (
                        <>
                          <Upload size={16} className="text-[#c5c5c5] mb-2" strokeWidth={1.5} />
                          <p 
