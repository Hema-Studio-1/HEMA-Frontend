...
export function CommentsPanel({
  ...
  const handlePost = () => {
    ...
    const comment: Comment = {
      ...
      assignedTo:
        showAssign && selectedAssignee !== ""
          ? teamMembers.find((m) => m.value === selectedAssignee)?.label
          : undefined,
    };
      ...
      >
            ...
            <div className="space-y-6">
              {filteredComments.map((comment) => (
                <div key={comment.id} className="flex gap-3">
                  {/* Avatar */}
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-[#A4AC96]/20 flex items-center justify-center">
                    <span
        ...
        >
          ...
          <textarea
            ...
            className="w-full px-4 py-3 bg-white rounded-sm text-[14px] text-foreground placeholder:text-textSecondary resize-none focus:outline-none focus:ring-1 focus:ring-[#A4AC96]/30"
            style={{
              fontFamily: "'Inter', sans-serif",
              fontWeight: 300,
              lineHeight: "1.6",
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-sm transition-colors duration-300 ${
                  showAssign
                    ? "bg-[#A4AC96]/10 text-foreground"
                    : "hover:bg-[#E8E6E3] text-[#626262]"
                }`}
                style={{
                  border: showAssign
                    ? "1px solid rgba(164, 172, 150, 0.2)"
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-sm transition-colors duration-300 ${
                newComment.trim()
                  ...
                  : "bg-[#E8E6E3] text-textSecondary cursor-not-allowed"
              }`}
            >
              <span
                className="text-[13px]"
          ...
          {/* Hint */}
          <p
            className="text-[11px] text-textSecondary mt-2"
            style={{
              fontFamily: "'Inter', sans-serif",
              fontWeight: 300,
            }}
